...
def CheckURI(uri, my_timeout=5):
    ...
    try:
        r = requests.get(uri, timeout=my_timeout)
...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Missing arguments".encode())
            if "longuri" not in params:
                self.wfile.write("long URI was not defined".encode())
            if "shortname" not in params:
                self.wfile.write("short name was not defined".encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("The url to redirect cannot be reached".encode())
